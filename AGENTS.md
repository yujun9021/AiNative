...
- **총괄 채팅**: 먼저 `AGENTS.md`, `docs/workflow/COORDINATOR.md`, `docs/workflow/ROADMAP.md`를 읽는다. 사용자 인터뷰, 프로젝트 기준, 기술·아키텍처, 개발환경, 작업 구성, 우선순위와 의존성을 관리한다. 제품 기능을 구현하지 않는다.
- **작업 채팅**: 먼저 `AGENTS.md`, `docs/workflow/WORK_CHAT.md`, `docs/workflow/ROADMAP.md`, 해당 `work/AN-xxx-slug/` 문서와 현재 변경 상태를 읽는다. 전달받은 하나의 `AN-xxx`만 담당하고 `intent → spec → plan → 구현 → evidence → 사용자 인수`를 수행한다.
- 작업 중 다른 기능이나 공통 구조에 영향을 주는 변경이 발견되면 구현을 멈추고 총괄 채팅에 결정을 요청한다.
